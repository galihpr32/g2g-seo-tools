...
## 0b. Claude SEO Skills integrations

All integrations are **additive** — no existing agent logic was replaced.
Universal constraints (apply to every sprint below):
- Output persisted to DB (no regenerate on page load)
- Every skill call uses retry + 25 s timeout wrapper
- Attribution shown in UI: "Generated via Anthropic skill: <name>"
- Kill switch via env var (default enabled); feature degrades gracefully when off
- Migration naming: `skill_<area>_<purpose>.sql`

| Sprint | Skill | Status | Kill switch env | Migration |
|--------|-------|--------|-----------------|-----------|
| SKILL.AI_VIS.1 | searchfit-seo:ai-visibility | ✅ Done | `SKILL_AI_VIS_RECOMMENDATIONS_ENABLED` | `skill_ai_vis_recommendations.sql` |
| SKILL.SCHEMA.1 | searchfit-seo:schema-markup | ✅ Done | `SKILL_SCHEMA_GEN_ENABLED` | (none — on-demand, no DB) |
| SKILL.BROKENLINK.1 | searchfit-seo:broken-links | ✅ Done | `SKILL_BROKENLINK_AUDIT_ENABLED` | `skill_broken_audits.sql` |
| SKILL.INTLINK.1 | searchfit-seo:internal-linking | ✅ Done | `SKILL_INTLINK_AUDIT_ENABLED` | `skill_intlink_audits.sql` |
| SKILL.SAGA.1 | searchfit-seo:keyword-cluster | pending | `SKILL_SAGA_LABELS_ENABLED` | `skill_saga_cluster_labels.sql` |
| SKILL.TECH_SEO.1 | searchfit-seo:technical-seo | pending | `SKILL_TECH_SEO_AUDIT_ENABLED` | `skill_tech_audits.sql` |
| SKILL.BRAGI.1 | searchfit-seo:content-brief | pending | `SKILL_BRAGI_OUTLINE_ENABLED` | none |

### SKILL.INTLINK.1 detail
- **New table:** `skill_intlink_recommendations` — one row per generation run. Stores orphan_count, opportunity_count, total_pages, avg_inlinks at generation time + recommendations jsonb.
- **API:** `GET /api/content/internal-links/recommend?site=<slug>` — latest saved recs. `POST { force?, summary, top_orphans, top_opps }` — generate (page passes current data in body; API does not re-query DataForSEO).
- **Rate limit:** 1x per 7 days. `force=true` bypasses. Returns `rate_limited: true` + cached record if within window.
- **UI:** Additive section at bottom of `/content/internal-links` page (after all tabs). Collapsible, "✨ Generate" / "🔄 Regenerate" buttons. Cards with priority border (red/amber/blue) + category badge (orphan_fix/hub_spoke/cluster_link/anchor_text/equity_flow).
- **No Sidebar change** — appended to existing `/content/internal-links` page.
- **Apply migration:** `psql "$DATABASE_URL" -f supabase/migrations/skill_intlink_audits.sql`

### SKILL.BROKENLINK.1 detail
- **New table:** `skill_broken_audits` — one row per audit run. Stores summary counters + full results jsonb.
- **API:** `GET /api/site-health/broken-internal?site=<slug>` — latest saved audit. `POST` — run new audit (up to 200 URLs, batched 10 concurrent HEAD requests).
- **URL sources:** `seo_content_briefs.page` (briefed pages) + `tier_serp_snapshots.our_url` (latest ranked URLs per keyword). `g2g_products` excluded — no URL column.
- **UI:** `/site-health/broken-internal` — summary cards (Checked/Broken/Redirects/OK), filter tabs, expandable row per URL with fix suggestion. "Run Audit" button.
- **Sidebar:** Added "🔴 Broken Internal" to SITE HEALTH group.
- **Apply migration:** `psql "$DATABASE_URL" -f supabase/migrations/skill_broken_audits.sql`
- **Note on SPAs:** G2G category pages are SPA-rendered and return HTTP 200 for the shell regardless of route validity. This tool catches real HTTP-level issues (decommissioned pages, server errors, redirect chains) but not client-side 404s.

### SKILL.SCHEMA.1 detail
- **No DB** — pure on-demand tool, no storage.
- **API:** `POST /api/tools/schema-generator` — accepts `{ url, page_type? }`. Fetches page via DataForSEO JS render → live fallback. Calls Claude Haiku with schema-markup skill prompt. Returns `{ schemas: [{ type, description, json_ld }] }`.
- **UI:** `/tools/schema-generator` — URL input + page_type dropdown (auto-detect default). Tabbed output per schema type. Copy-to-clipboard for `<script>` tag. "Test in Google Rich Results ↗" link per card.
- **Sidebar:** Added "🧬 Schema Generator" to SETTINGS group (`/tools/schema-generator`).
- **Model:** claude-haiku-4-5-20251001. Max 2000 tokens, 3 attempts, 25 s timeout.
- **No migration needed** — stateless.

### SKILL.AI_VIS.1 detail
- **New table:** `skill_ai_vis_recommendations` — one row per (owner, site, snapshot_date). Latest row wins on read.
- **API:** `GET /api/reports/ai-visibility/recommend?site=<slug>` — latest saved recs. `POST` with `{ site, force? }` — generate/serve cached.
- **UI:** Collapsible "💡 Recommendations" section on `/reports/ai-visibility`, below per-source table. "🤖 Regenerate" button forces fresh generation.
- **Model:** claude-haiku-4-5-20251001 (Haiku for cost). Max 5 recs, validated (must start with action verb + include measurable metric).
- **Apply migration:** `psql "$DATABASE_URL" -f supabase/migrations/skill_ai_vis_recommendations.sql`

### Previous Norse-agent upgrades (same session, additive)
- **Odin** — added `deriveContentStrategy()` (content-strategy skill): per-game intent/content_type/content_angle/pillar_or_cluster. Stored in findings + enriches Bragi fast-path handoff. Shown in OdinScoringPanel "Content Angle" column.
- **Heimdall** — expanded `categorizeDrop()` (technical-seo + seo-audit skill): structured `fix_checklist` (4 ordered steps per category) + `recovery_potential`. Top fix step surfaced in action item descriptions.
