...
/**
 * Kang Cilok — Off-Page / Outreach Agent
 *
 * Logic:
 ...
 */
export async function runKangCilok(
  ownerId: string,
  siteSlug: string,
  runId: string
  ...
  try {
    ...
    for (const action of gapActions) {
      ...
      if (competitorDomain && !skipDomains.has(competitorDomain) && !candidateDomains.find(c => c.domain === competitorDomain)) {
        candidateDomains.push({ domain: competitorDomain, position: Number(d.competitor_position ?? 5), source: 'intel-bakso' })
      }
...
async function _finishRun(
  ...
  }).eq('owner_user_id', ownerId).eq('agent_key', 'kang-cilok')
}
