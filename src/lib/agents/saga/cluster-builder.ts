...
export async function runClusterBuilder(
  ...
  for (const cand of unique) {
    ...
    const brandSlug = slugify(cls.brand)
    ...
    if (!brand) {
      ...
      if (brandErr || !inserted) {
        ...
        const { data: refetch } = await db
          ...
          .maybeSingle()
        if (!refetch) {
          result.warnings.push(`Brand insert failed for "${cls.brand}": ${brandErr?.message ?? 'unknown'}`)
          result.keywordsSkipped++
          continue
        }
    ...
    if (!sub) {
      const { data: inserted, error: subErr } = await db
        ...
        .insert({
          ...
          auto_generated: true,
          source:         'saga',
          description:    `Sub-product under "${cls.brand}". Auto-generated by Saga.`,
        })
        .select('id, topic, topic_slug, level, parent_map_id')
        .maybeSingle()
      if (subErr || !inserted) {
