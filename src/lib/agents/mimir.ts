...
/**
 * Mimir — Config Tuner (Norse god of wisdom)
 *
 ...
 *
 * What Mimir tunes (initial heuristics — kept conservative):
 *
 ...
 *
 * 2. LOKI.svGapPriority (NEW — synthesised from action priorities)
 *    - If >70% of low-priority gap actions get rejected → suggest raising the
 *      "low" volume cutoff from 1000 → 2000.
 *
 * 3. ODIN.priorityScoreThreshold (synthesised)
 ...
 *      published WITHOUT user editing notes → suggest LOWER minScore (Tyr is
 *      being too strict).
 *    - If >30% of auto-promoted briefs get manually demoted by the user
 *      back to 'draft' → suggest HIGHER minScore (Tyr is being too lenient).
 *
 * Mimir does NOT auto-apply. Every suggestion goes through the approval queue.
 */
...
export const MIMIR_DEFAULTS: MimirConfig = {
  windowDays:           30,
  minSampleSize:        10,
...
export async function runMimir(
  ownerId: string,
  siteSlug: string,
  runId: string,
  config: Partial<MimirConfig> = {}
): Promise<{ summary: string; actionsQueued: number }> {
  const cfg = { ...MIMIR_DEFAULTS, ...config }
  ...
  try {
    ...
    for (const { agent_key, suggestion } of suggestions) {
      ...
      if (insertErr) {
        console.error(`[mimir] failed to queue tune_config for ${agent_key}:`, insertErr.message)
...
function analyseHeimdall(
  ...
  const lowPri = heimActions.filter(a => a.priority === 'medium')   // heimdall uses 'medium' as default low-tier
  ...
  if (rejectRate > cfg.approvalRateThresh) {
    // tighten
    const newClicks = Math.round(minClicksDrop * 1.4)   // +40% for clearer signal
...
function analyseLoki(
  ...
  if (rejectRate > 0.7) {
    // Suggest raising the priority cutoff
...
async function analyseTyr(
  ...
  // Read briefs reviewed by Tyr in the window
  ...
  // How many of Tyr's auto-promoted briefs got demoted back to 'draft' by user?
  const demotedRate = autoPromoted.filter(b => b.status === 'draft').length / autoPromoted.length
  // How many got published without manual editing? (proxy: status='published')
