...
/**
 * Intel Bakso — Competitive Analysis Agent
 *
 * Logic:
 ...
 */
export async function runIntelBakso(
  ownerId: string,
  siteSlug: string,
  runId: string
): Promise<{
  ...
  try {
    ...
    try {
      ...
      console.warn('[intel-bakso] Keyword gap analysis failed:', gapErr)
    }
    ...
    try {
      ...
      console.warn('[intel-bakso] SOV analysis failed:', sovErr)
    }
    ...
    if (hasCrawlAPI) {
      try {
        ...
        console.warn('[intel-bakso] FireCrawl analysis failed:', crawlErr)
      }
    }
