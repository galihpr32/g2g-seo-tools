...
import { retrieveMemories, extractMemoriesFromConversation } from '@/lib/agents/mimir-memory'
...
export async function POST(req: Request) {
  ...
  // ── Memory retrieval — inject "what I remember" block into system prompt ──
  // Hint tokens come from the page-context kind + any topic/relation pulled
  // through (best-effort string keys). Cheap match against memory tags.
  const ctxRecord = pageContext as unknown as Record<string, unknown>
  const topicSlugHint = typeof ctxRecord.topicSlug === 'string'
    ? ctxRecord.topicSlug
    : typeof ctxRecord.topic_slug === 'string'
      ? ctxRecord.topic_slug
      : null
  const relationIdHint = typeof ctxRecord.relationId === 'string'
    ? ctxRecord.relationId
    : typeof ctxRecord.relation_id === 'string'
      ? ctxRecord.relation_id
      : null

  const hintTokens: string[] = [pageContext.kind]
  if (topicSlugHint)  hintTokens.push(...topicSlugHint.split(/[-_\s]+/).filter(Boolean))
  // User's first message often contains the topic — pull keywords.
  hintTokens.push(
  ...
  // ── Fire-and-forget memory extraction (after response is sent) ──────────
  // Latency-sensitive: we don't make the user wait on Haiku to extract durable
  // memories. Uses Vercel after() so the function keeps running post-response.
  const conversationIdForExtract = convo.id
  after(
    extractMemoriesFromConversation(db, {
      ownerId,
      conversationId: conversationIdForExtract,
      siteSlug,
      topicSlug:      topicSlugHint,
      relationId:     relationIdHint,
      // Only feed the LAST exchange (user msg + assistant reply) — that's
      // where new memories surface. Older turns were already extracted on
      // their own request, so re-running over them is wasted Haiku calls.
      transcript: [
        { role: 'user',      content: message },
        { role: 'assistant', content: reply   },
      ],
    }).catch(e => console.error('[mimir-chat] memory extract failed:', e))
  )
