...
async function fetchSemrushBalance(): Promise<{
  ...
  if (!key) return { units_remaining: null, error: 'API key not set' }

  try {
    const url = `https://api.semrush.com/?type=api_units&key=${key}`
    const res = await fetch(url, { signal: AbortSignal.timeout(10000) })

    if (!res.ok) {
      return { units_remaining: null, error: `HTTP ${res.status}` }
    }

    const text = (await res.text()).trim()
    const units = parseInt(text, 10)

    return { units_remaining: isNaN(units) ? null : units }
  } catch (err) {
