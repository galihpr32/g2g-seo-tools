  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      <div className="flex flex-wrap items-center gap-2 mb-5">
        ...
        <button
          onClick={() => window.open(`/api/feedback/export?format=markdown&status=${filter === 'all' ? 'all' : filter}`, '_blank')}
          title="Open a markdown dump of all feedback tickets — paste into Claude for triage."
          className="ml-auto text-xs text-blue-300 hover:text-blue-200 border border-blue-800/60 px-3 py-1.5 rounded-lg transition"
        >
          📋 Export for Claude
        </button>
        <button onClick={load} className="text-xs text-gray-500 hover:text-white border border-gray-700 px-3 py-1.5 rounded-lg transition">
          ↻ Refresh
        </button>
      </div>
